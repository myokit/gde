...
import gde
from .qt import Qt, QtCore, QtGui, QtWidgets, Application
...
# Settings file
SETTINGS_FILE = os.path.join(gde.DIR_USER, 'gde.ini')
...
ABOUT = '<h1>' + TITLE + '</h1>' + """
<p>
    Graph Data Extractor (GDE) is an application for extracting raw data points
    from an image file.
</p>
<p>
...
def icon():
    ...
    icon = QtGui.QIcon()
    for i in icons:
        icon.addFile(os.path.join(gde.DIR_DATA, 'gui', i))
    return icon
...
class DocumentVariable(QtCore.QObject):
    ...
    def _value_from_string(self, value):
        ...
        if self._vtype == V_INT:
            ...
            return min(1, max(0, float(value) if value else 0))
        ...
        try:
            ...
            if doc:
                ...
                def pt(axis, name, idx):
                    ...
                    if y is not None:
                        d[idx][0] = y.attrib['value']
                    y = find(x, T_VARIABLE, 'y')
                    if y is not None:
                        d[idx][1] = y.attrib['value']
                    y = find(x, T_VARIABLE, 'value')
                    if y is not None:
                        d[idx][2] = y.attrib['value']
    ...
    def __init__(self, x, y=None):
        if isinstance(x, Point2D):
            ...
            raise AttributeError(
                'Point2D can only be created with Point2D(float, float) or'
                ' Point2D(Point2D).')
...
class Line2D(object):
    ...
    def __init__(self, a, b=None, c=None, d=None):
        if isinstance(a, Line2D):
            ...
            raise AttributeError(
                'Invalid signature. Expecting Line2D(Line2D) or '
                'Line2D(Point2D, Point2D) or'
    ...
    def intersect(self, other):
        ...
        if abs(d) < 1e-14:
            raise AttributeError(
                'Lines are too close to parallel to detect intersection.')
        d = 1 / d
        c1 = x1 * y2 - y1 * x2
        c2 = x3 * y4 - y3 * x4
        return Point2D(
...
class DocumentAction(object):
    ...
    def perform(self):
        ...
        if self._performed:
            raise Exception(
                'Action failed: Action cannot be performed twice.')
...
class DA_AddVariable(DocumentAction):
    ...
    def _perform(self):
        if self.variable is not None:
            ...
            self.variable = self.parent.silent_add_variable(
                self.vtype, self.name, self.value)
...
#
#
class GraphDataExtractor(Application):
    """
    ...
    def action_add_data_point(self, x=0.5, y=0.5):
        ...
        p = data.add_child(
            T_DATA_POINT, name, ((V_NORM, 'x', x), (V_NORM, 'y', y)))
        p.select()
        return p
    ...
    def action_extract(self):
        ...
        fname = QtWidgets.QFileDialog.getSaveFileName(
            ...
            self._path,
            filter='Comma-separated file (*.csv *.txt)')[0]
        if fname:
            fname = str(fname)
            if fname:
    ...
    def action_license(self):
        ...
        """
        self.clear_focus()
        QtWidgets.QMessageBox.about(self, TITLE, gde.LICENSE_HTML)
    ...
    def action_set_image(self):
        ...
        self.clear_focus()
        fname = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Open image file', self._path)[0]
        if fname:
            fname = str(fname)
            ...
            if self._file:
                ...
                if fpath[:n] == cpath:
                    fname = os.path.join(
                        fpath[1 + n:], os.path.basename(fname))
            if fname != '':
    ...
    def clear_focus(self):
        ...
        if w is not None and isinstance(w, DocumentVariableField):
            w.clearFocus()
    ...
    def create_menu(self):
        ...
        self._tool_saveas.setStatusTip(
    ...
    def handle_mouse_moved(self, x, y):
        ...
        self._label_cursor.setText(
            '(' + F.format(x) + ', ' + F.format(y) + ') :: (' + F.format(xx)
            + ', ' + F.format(yy) + ')')
    ...
    def handle_node_selected(self, node):
        ...
        if node.get_ntype() == T_AXIS_REFERENCE_POINT:
            self.statusBar().showMessage(
                'Drag to move. Hold Ctrl to create orthogonal axis.')
        else:
...
class AxisItem(SceneItem):
    ...
    def init(self, scene, node):
        ...
        # Update coordinates
        if self._r1 is not None and self._r2 is not None:
            self.update_coords()
    ...
    def update_coords(self):
        ...
        """
        scene = self.scene()
        if scene is not None and self._r1 is not None and self._r2 is not None:
